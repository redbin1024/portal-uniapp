...
function generate({ children }, { slot, event, scopeId, emitFile, filename, directive, lazyElement, isBuiltInComponent, isMiniProgramComponent, checkPropName, component, autoImportFilters, filter, }) {
    const context = {
        slot,
        event,
        code: '',
    ...
    if (filter && filter.generate && autoImportFilters.length) {
        autoImportFilters.forEach((autoImportFilter) => {
            context.code +=
                filter.generate(autoImportFilter, '/common/' + autoImportFilter.id) + '\n';
        });
    }
